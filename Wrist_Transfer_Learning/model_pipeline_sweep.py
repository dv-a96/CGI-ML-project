...
def model_pipeline(config):
    ...
    if existing_data.empty:
        ...
        model, train_loader, test_loader, criterion, optimizer, gi_dataset = make(config)
        ...
        for num_of_epoch in [5, 5, 5]:
            ...
            acc = test(model, test_loader, criterion, 16, gi_dataset.getdataframe())
...
def make(config):
    batch_size = 16
    # Make the data
    train_loader, test_loader, gi_dataset = make_loaders(batch_size, config)
    ...
    return model, train_loader, test_loader, criterion, optimizer, gi_dataset
def make_loaders(given_batch_size, config):
    ...
    # Split the reshape_size string into two separate dimensions
    reshape_dims = config.reshape_size.split('_')
    reshape_height = reshape_dims[0]
    reshape_width = reshape_dims[1]

    # Construct the CSV file path
    path_ending = f"{config.num_of_measurements}_{reshape_height}_{reshape_width}.csv"
    csv_path = f"../Processed_Dataset/new_dataset_{path_ending}"
    ...
    return train_loader, test_loader, wrist_gi_dataset
